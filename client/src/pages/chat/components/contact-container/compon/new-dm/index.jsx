...
const NewDM = () => {
  ...
  const searchContacts = async(searchTerm)=>{
     try {
      ...
      setSearchedContacts([]); // Optionally clear search results
      return; // ⛔ stop execution early
    }
          res = await apiClient.post(SEARCH_CONTACTS_ROUTES, {searchTerm},{withCredentials:true})
    ...
    <>
      ...
      <Dialog open={openNewContactModal} onOpenChange={setOpenNewContactModal} >
        ...
        <DialogContent className="bg-[#181920] border-none text-white w-[400px] h-[400px] flex flex-col ">
         ...
         { searchedContacts.length>0 && ( <ScrollArea className="h-[250px]">
             <div className="flex flex-col gap-5">
                 {
                   searchedContacts.map((contact)=>(
                    ...
                    >
                        {
                            contact.image ? <AvatarImage src={`${HOST}/${contact.image}`} alt="profile" className="object-cover w-full h-full bg-black " /> : <div className={`uppercase h-12 w-12  text-lg border-[1px] flex items-center justify-center rounded-full ${getColor(
                                contact.color
                            )}`}>
          ...
          {
            searchedContacts.length<=0 &&  <div className='flex-1 md:bg-[#1c1d25] md:flex flex-col justify-center items-center  duration-1000 transition-all'>
      <Lottie
      ...
      <div className='text-opacity-80 flex flex-col gap-5 items-center mt-5 lg:text-2xl text-xl transition-all duration-300 text-center '>
         <h3 className='poppins-medium '>
             <span className='text-purple-500'></span> Search
